...
A(1,1:4)=[-20,6,4,-1]/(12*h^2);
A(end,end-3:end)=[-1,4,6,-20]/(12*h^2);
%設定A矩陣

F=exp(x);
F(2)=exp(1/m)-(11*a)/(12*h^2);
F(3)=exp(2/m)+a/(12*h^2);
F(end-2)=exp(1-2/m)+b/(12*h^2);
F(end-1)=exp(1-1/m)-(11*b)/(12*h^2);
F2=F';
F2(1,:)=[];
F2(end,:)=[];
U = A\F2;
...
% plot(x,U2,'.g',x,exp(x) + (-5-exp(0))*(x - 1) + 1 - exp(1))
...
E = [3.27E-08, 1.03E-09, 3.68E-11, 9.93E-13];
h = hval';
E2 = E';
ntest = length(h);
loglog(h,E2,'o-')
